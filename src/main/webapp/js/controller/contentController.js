...
app.controller('contentController' ,function($scope,$controller,contentService){	
	...
	// select2 查询所有的分类
	$scope.findAllCatagory = function(){
		contentCategoryService.findAll().success(
				function(response){
					$scope.selctlist = response;
				}
		)
	}
	...
	// 根据categoryId查询首页大广告
	$scope.contentList = []
	$scope.findContentByCatId = function(categoryId){
		contentService.findContentByCatId(categoryId).success(
				function(response){
